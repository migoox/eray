...
int main() {
  ...
  // -- Application ----------------------------------------------------------------------------------------------------
  auto app = vkren::VulkanApplication(nullptr);
  {
    struct AppData {
      vkren::BufferResource vert_buffer;
      vkren::BufferResource ind_buffer;

      vkren::ImageResource txt_image;
      vk::raii::ImageView txt_view  = nullptr;
      vk::raii::Sampler txt_sampler = nullptr;

      std::vector<void*> uniform_buffers_mapped;
      std::vector<vkren::BufferResource> uniform_buffers;
    ...
    auto app_create_info = vkren::VulkanApplicationCreateInfo{
        .app_name    = "Depth Buffer",
        .enable_msaa = true,
        .vsync       = false,
        .on_init =
            [&app_data](vkren::VulkanApplicationContext& ctx) {
              // -- Buffers setup
              // ----------------------------------------------------------------------------------------
              {
                ...
                app_data.vert_buffer =
                    vkren::BufferResource::create_vertex_buffer(ctx.device_, vertices_region.size_bytes())
                        .or_panic("Could not create the vertex buffer");
                app_data.vert_buffer.write(vertices_region).or_panic("Could not fill the vertex buffer");

                auto indices_region = eray::util::MemoryRegion{vb.indices.data(), vb.indices_size_bytes()};
                app_data.ind_buffer =
                    vkren::BufferResource::create_index_buffer(ctx.device_, indices_region.size_bytes())
                        .or_panic("Could not create a Vertex Buffer");
                app_data.ind_buffer.write(indices_region).or_panic("Could not fill the index buffer");
                ...
                app_data.uniform_buffers_mapped.clear();
                vk::DeviceSize size_bytes = sizeof(UniformBufferObject);
                for (auto i = 0; i < vkren::VulkanApplicationContext::kMaxFramesInFlight; ++i) {
                  ...
                  app_data.uniform_buffers.emplace_back(std::move(ubo.buffer));
                  app_data.uniform_buffers_mapped.emplace_back(ubo.mapped_data);
                }
              }
              ...
              // -----------------------------------------------------------------------------------------
              {
                ...
                app_data.txt_image =
                    vkren::ImageResource::create_texture(ctx.device_, vkren::ImageDescription::from(img))
                        .or_panic("Could not create a texture image");
                app_data.txt_image.upload(img.memory_region()).or_panic("Could not upload the image");
                app_data.txt_view = app_data.txt_image.create_image_view().or_panic("Could not create the image view");
                ...
                app_data.txt_sampler =
                    vkren::Result(ctx.device_->createSampler(sampler_info)).or_panic("Could not create the sampler");
              }
              ...
              {
                auto result =
                    vkren::DescriptorSetBuilder::create(ctx.dsl_manager_, ctx.dsl_allocator_)
                        ...
                        .build_many(vkren::VulkanApplicationContext::kMaxFramesInFlight)
                        .or_panic("Could not create descriptor sets");

                app_data.descriptor_sets = result.descriptor_sets;
                app_data.dsl             = result.layout;

                auto writer = vkren::DescriptorSetWriter::create(ctx.device_);
                for (auto i = 0U; i < vkren::VulkanApplicationContext::kMaxFramesInFlight; ++i) {
                  writer.write_buffer(0, app_data.uniform_buffers[i].desc_buffer_info(),
                                      vk::DescriptorType::eUniformBuffer);
                  writer.write_combined_image_sampler(1, app_data.txt_view, app_data.txt_sampler,
                                                      vk::ImageLayout::eShaderReadOnlyOptimal);
                  writer.write_to_set(app_data.descriptor_sets[i]);
                  writer.clear();
                }
              }
              ...
              // --------------------------------------------------------------------------
              {
                auto main_binary = eray::res::SPIRVShaderBinary::load_from_path(eray::os::System::executable_dir() /
                                                                                "shaders" / "main.spv")
                                       .or_panic("Could not find main_sh.spv");
                auto main_shader_module = vkren::ShaderModule::create(ctx.device_, main_binary)
                ...
                auto pipeline = vkren::GraphicsPipelineBuilder::create(ctx.swap_chain_)
                                    ...
                                    .with_descriptor_set_layout(app_data.dsl)
                                    .with_depth_test()
                                    .with_blending()
                                    .build(ctx.device_)
                                    .or_panic("Could not create a graphics pipeline");

                app_data.graphics_pipeline        = std::move(pipeline.pipeline);
                app_data.graphics_pipeline_layout = std::move(pipeline.layout);
              }
            },
        .on_frame_prepare =
            [&app_data](vkren::VulkanApplicationContext& ctx, uint32_t image_index) {
              ...
              ubo.proj  = eray::math::perspective_vk_rh(
                  eray::math::radians(80.0F),
                  static_cast<float>(window_size.width) / static_cast<float>(window_size.height), 0.01F, 10.F);

              memcpy(app_data.uniform_buffers_mapped[image_index], &ubo, sizeof(ubo));
            },
        .on_record_graphics =
            [&app_data](vkren::VulkanApplicationContext& ctx, vk::raii::CommandBuffer& graphics_command_buffer,
                        uint32_t image_index) {
              // We can specify type of the pipeline
              graphics_command_buffer.bindPipeline(vk::PipelineBindPoint::eGraphics, app_data.graphics_pipeline);
              graphics_command_buffer.bindVertexBuffers(0, app_data.vert_buffer.buffer(), {0});
              graphics_command_buffer.bindIndexBuffer(app_data.ind_buffer.buffer(), 0, vk::IndexType::eUint16);
              ...
              graphics_command_buffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                                         app_data.graphics_pipeline_layout, 0,
                                                         app_data.descriptor_sets[image_index], nullptr);

              graphics_command_buffer.drawIndexed(12, 1, 0, 0, 0);
            },
    };

    app = vkren::VulkanApplication::create(std::move(app_create_info));
    app.run();
  }
