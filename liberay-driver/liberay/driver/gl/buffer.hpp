 ...
 public:
  ...
  class Layout {
   public:
    ...
    template <CPrimitiveType PrimitiveType>
    auto add_attribute(util::zstring_view name, size_t location, size_t count, bool normalize = false) {
    ...
    const Attribute& attribute(util::zstring_view name) const { return attribs_.at(indices_.at(name)); }
   ...
   private:
    std::unordered_map<util::zstring_view, size_t> indices_;
   ...
   */
  template <CPrimitiveType PrimitiveType>
  void set_attribute_value(GLuint vert_start_index, util::zstring_view attr_name, const PrimitiveType* attr_value) {
